...
void* handle_client(void* args) {
    client_info* client = (client_info*)args;   // Cast the argument to client_info structure
    int client_socket = client->socket;         // Extract the client's socket descriptor
    ...
    // Close the client socket and free the allocated memory for client information
    close(client_socket);
    free(client);  // Free the dynamically allocated client structure

    // End the thread after handling the client
    pthread_exit(NULL);
}
...
int main() {
    ...
    pthread_t thread_id;                             // Thread ID for handling client connections
