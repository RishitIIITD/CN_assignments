...
// Driver Code to create multiple client threads and initiate concurrent connections
int main(int argc, char *argv[]) {
    ...
    // Wait for all threads (clients) to finish their tasks
    for (int i = 0; i < num_clients; i++) {
        pthread_join(threads[i], NULL);  // Wait for each thread to complete
    }

    free(threads);  // Free dynamically allocated memory after threads complete
    return EXIT_SUCCESS;  // Return success after all client threads have been handled
}